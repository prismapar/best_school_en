$(document).ready(function(){
    ...
    $("#department,#municipality,#studentsNumber,#studentsNumberSaber,#schoolName").change(function () {
       //getTop()
       filterSchools()
    });  
...
var formatCurrency = d3.format("$,.0f"),
    formatNumber = d3.format(".0f"), // d3.format(",.0f"),
...
d3.csv("data_school.csv",
    ...
    //console.log("total schools: " + all_schools.length);
   
    //load_table_visualization(all_schools);
    //Cargar primero 100 colegios
    school_top = $("#top").val();
...
function loadSliders()
{
    ...
    group1.call(slider1);
    d3.select("p#value1").text(d3.format('.0%')(slider1.value()))
    d3.select("a#setValue1").on("click", () => { slider1.value(0.33); d3.event.preventDefault(); });

}
*/

/*
function calculateWeightFormula1(val)
{
    // d3.select("p#value1").text(d3.format('.0%')(val));
     //console.log(d3.format('.0%')(val));
     //console.log(val * 100);

     changeFormulasWeight(1, val * 100);
}


function changeFormulasWeight(formula, weight)
{
    let weigth_form_1 = 0;
    let weigth_form_2 = 0;
    let weigth_form_3 = 0;
    var filtered_schools_temp = filtered_schools;

    if(formula == 1)
    {
        weigth_form_1 = weight;
        weigth_form_2 = weight == 100 ? 0 : (100 - weight)/2;
        weigth_form_3 = weight == 100 ? 0 : (100 - weight)/2;

        console.log("weigth_form_1: " + weigth_form_1);
        console.log("weigth_form_2: " + weigth_form_2);
        console.log("weigth_form_3: " + weigth_form_3);
    }
    else if(formula == 2)
    {
        weigth_form_1 = weight == 100 ? 0 : (100 - weight)/2;
        weigth_form_2 = weight;
        weigth_form_3 = weight == 100 ? 0 : (100 - weight)/2;
    }
    else if(formula == 3)
    {
        weigth_form_1 = weight == 100 ? 0 : (100 - weight)/2;
        weigth_form_2 = weight == 100 ? 0 : (100 - weight)/2;
        weigth_form_3 = weight;
    }


     for (var i = 0; i < filtered_schools_temp.length; i++) {
    
        filtered_schools_temp[i].formula_1 =  Math.round( ((filtered_schools_temp[i].formula_1 * weigth_form_1)/100) * 100 ) /100;
        filtered_schools_temp[i].formula_2 =  Math.round( ((filtered_schools_temp[i].formula_2 * weigth_form_2)/100) * 100 ) /100;
        filtered_schools_temp[i].formula_3 =  Math.round( ((filtered_schools_temp[i].formula_3 * weigth_form_3) / 100) * 100 ) /100;      
    }
...
function getSchoolByDepartment(){

    school_department = $("#department").val();
    console.log("filtered department: " + school_department);
    
    filtered_schools = filtered_schools.filter(filterByDepartment);
    console.log("filtered schools: " + filtered_schools.length);
    load_table_visualization(filtered_schools);   
...
{
    $("#municipality").append('<option value="ALL">ALL</option>'); 
...
function load_schools()
{
   ...
   // $("#schools").append('<option value="TODOS">TODOS</option>'); 
    $('#schools').append('<option value="ALL" selected hidden>ALL</option>');   
...
function loadParalelCoordinates (datos){
    ...
    var margin = { left:500, right:40, top:30, bottom:20 };
...
function loadBarcharVisualization(school_data)
{
    ...
    console.log(school_data);
     ...
     for (var i = 0; i < school_data.length; i++) {
        ...
        obj.formula_1 = +school_data[i].formula_1; 
  ...
  g.append( "g" )
    ...
    .enter().append( "rect" )
      ...
      //.attr( "x", d => x1( d.key ) )
      .attr("y", function(d) { return y(d.formula_1); })      
      //.attr( "y", d => y( d.value ) )
